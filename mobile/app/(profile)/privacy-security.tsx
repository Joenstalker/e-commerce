...
function PrivacyAndSecurityScreen() {
  ...
  const handleChangePassword = async () => {
    const url = process.env.EXPO_PUBLIC_CLERK_ACCOUNT_URL;
    if (!url) {
      Alert.alert(
        "Change password",
        "Account management is not configured. Set EXPO_PUBLIC_CLERK_ACCOUNT_URL in .env to your Clerk account URL, then change your password securely in the browser."
      );
      return;
    }
    try {
      await WebBrowser.openBrowserAsync(url, { presentationStyle: 1 });
    } catch (e: any) {
      Alert.alert("Error", e?.message ?? "Could not open account settings.");
    }
  };
        ...
        <View className="px-6 pt-6">
          ...
          {securitySettings.map((setting) => (
            <TouchableOpacity
              ...
              onPress={
                setting.type === "navigation" && setting.id === "password"
            ...
            >
              <View className="flex-row items-center">
                <View className="bg-primary/20 rounded-full w-12 h-12 items-center justify-center mr-4">
                  <Ionicons name={setting.icon as any} size={24} color="#F97316" />
                </View>
