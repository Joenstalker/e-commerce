...
const CATEGORIES = [
  ...
  { name: "Electronics", image: require("@/assets/images/electronic.png") },
      ...
      >
        ...
        <View className="px-6 pb-4 pt-6">
          <View className="flex-row items-center justify-between mb-6">
            ...
            <TouchableOpacity className="bg-surface p-3 rounded-full border border-surface-lighter" activeOpacity={0.7}>
              <Ionicons name="options-outline" size={22} color={"#57534E"} />
            </TouchableOpacity>
          </View>

          {/* SEARCH BAR */}
          <View className="bg-surface flex-row items-center px-5 py-4 rounded-2xl border border-surface-lighter">
            <Ionicons color={"#78716C"} size={22} name="search" />
            <TextInput
            ...
            {CATEGORIES.map((category) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  className={`mr-3 rounded-2xl size-20 overflow-hidden items-center justify-center border ${isSelected ? "bg-primary border-primary" : "bg-surface border-surface-lighter"}`}
                >
                  {category.icon ? (
                    <Ionicons
                      name={category.icon}
